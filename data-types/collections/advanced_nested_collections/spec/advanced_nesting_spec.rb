...
RSpec.describe 'Advanced Nested Collections' do
  ...
  it 'test 2' do
    ...
    pancake_ingredients = stores[:dennys][:dishes]
                                .find{ |dish| dish[:name] == "Pancakes"}[:ingredients]
  ...
  it 'test 3' do
    ...
    risotto_price = stores[:olive_garden][:dishes]
                          .find{ |dish| dish[:name] == "Risotto"}[:price]
  ...
  it 'test 4' do
    ...
    big_mac_ingredients = stores[:macdonalds][:dishes].find { 
                            |dish| dish[:name] == 'Big Mac'}[:ingredients]
  ...
  it 'test 7' do
    ...
    # all restaurants
    employee_names = stores.keys.map { |key| stores[key][:employees] }.flatten
  ...
  it 'test 8' do
    ...
    ingredients = stores.map { |k,v| v[:dishes].map{ |k| k[:ingredients]}}.flatten
  ...
  it 'test 9' do
    ...
    full_menu_price = stores[:olive_garden][:dishes].sum { |item| item[:price] }
  ...
  it 'test 10' do
    ...
    olive_garden_menu = stores[:olive_garden][:dishes].map { |dish| {dish[:name] => dish} }.reduce({}, :merge)
  ...
  it 'test 11' do
    ...
    full_menu = stores.map { |_,v| v[:dishes].map { |dish| {dish[:name] => dish} }}.flatten.reduce({}, :merge)
